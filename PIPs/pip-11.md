...
This document proposes a standard for generating HD key chains for the BLS signature scheme based on [BIP-0032](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki).
...
* point<sub>G1</sub>(p): Returns the point in the G1 subgroup resulting from EC point multiplication of the G1 base point with the integer p.
* point<sub>G2</sub>(p): Returns the point in the G2 subgroup resulting from EC point multiplication of the G2 base point with the integer p.
...
* ser<sub>G1</sub>(P1): [Serializes](https://www.ietf.org/archive/id/draft-irtf-cfrg-pairing-friendly-curves-11.html#appendix-C) the point P1 as 48-byte sequence.
* ser<sub>G2</sub>(P2): [Serializes](https://www.ietf.org/archive/id/draft-irtf-cfrg-pairing-friendly-curves-11.html#appendix-C) the point P2 as 96-byte sequence.
* parse<sub>256</sub>(p): Interprets a 32-byte sequence as a 256-bit number, most significant byte first.

## Master key generation

To generate the master key, the [KeyGen](https://www.ietf.org/archive/id/draft-irtf-cfrg-bls-signature-05.html#section-2.3) procedure described in the BLS Signatures RFC is used.
...
Provided a parent extended key and an index i, calculating the corresponding child extended key is feasible. The specific algorithm used for this computation varies based on the child being a hardened key or not (or equivalently, whether i is greater than or equal to 2<sup>31</sup>), and on the type of keys involved, whether private or public.
...
For hardened key derivation, to obtain different private keys, we pad the private key with 0x01 in G<sub>1</sub> and 0x00 in G<sub>2</sub>.
